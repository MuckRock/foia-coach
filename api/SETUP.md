...
**`.envs/.local/.postgres`** — copy this as-is:
...
**`.envs/.local/.api`** — get the shared OpenAI API key from a teammate and fill it in:
...
Get the `foia_coach_seed_data.tar.gz` file from a teammate and place it in the `api/` directory:
...
```bash
./api/scripts/load_seed_data.sh
```

This extracts two directories into `api/` and loads the database:
